...
Router.map(function() {
  ...
  this.route('teacherTalk', {
    ...
    action: function() {
      if(Session.get("curClassroomId")){
        this.redirect('teacherTalkWithParam',{_id:Session.get("curClassroomId")});
      }
      else{
        this.render();
      }
  ...
  this.route('teacherWork', {
    ...
    action: function() {
      if(Session.get("curClassroomId")){
        this.redirect('teacherWorkWithParam',{_id:Session.get("curClassroomId")});
      }
      else{
        this.render();
      }
  ...
  this.route('/t/workPanel/:_id', function() {
    this.render('teacherWork', {
      data: function() {
        Session.set("curMode", Schemas.ticketType.talkTicket);
      }
    });
  }, {
    name: 'teacherWorkWithParam',
    layoutTemplate: 'tabsLayout'
  });
    ...
    if (isTeacherAccount(Meteor.userId())) {
      ...
      if(!!curClass){
        if(curClass.status !== Schemas.classroomStatus.close){
          ...
          ClassroomKicker.attendClass(curClass._id);
